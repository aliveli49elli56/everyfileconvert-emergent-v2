...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const hreflangs = getHreflangLinks("/background-remover");
  ...
  return {
    ...
    openGraph: {
      ...
      type: "website",
      url: `https://everyfileconvert.com/${locale}/background-remover`,
    },
    twitter: { card: "summary_large_image", title: meta?.backgroundRemoverTitle, description: meta?.backgroundRemoverDesc },
    alternates: {
      canonical: `https://everyfileconvert.com/${locale}/background-remover`,
...
const tools = [
  { name: "Background Remover", description: "Remove backgrounds from any image instantly with AI", icon: Eraser, href: "background-remover/remove", color: "from-indigo-500 to-violet-500", popular: true },
  { name: "Background Replacer", description: "Replace backgrounds with solid colors or images", icon: Palette, href: "background-remover/replace", color: "from-rose-500 to-pink-500" },
  { name: "Batch Background Remover", description: "Remove backgrounds from multiple images at once", icon: Layers, href: "background-remover/batch", color: "from-cyan-500 to-teal-500" },
  { name: "Transparent PNG Maker", description: "Convert any image to transparent PNG format", icon: Wand2, href: "background-remover/transparent", color: "from-emerald-500 to-green-500" },
  { name: "Portrait Mode", description: "Add blur effect similar to portrait mode photography", icon: ImagePlus, href: "background-remover/portrait", color: "from-amber-500 to-orange-500" },
];
...
export default async function LocaleBackgroundRemoverPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const breadcrumbJsonLd = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 3, "name": "Background Remover", "item": `https://everyfileconvert.com/${locale}/background-remover` },
    ],
  };
    ...
    <>
      ...
      <section className="py-16 lg:py-20">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center mb-12">
            <Badge variant="secondary" className="mb-4 px-3 py-1 bg-indigo-100 text-indigo-700 border-indigo-200">
              AI-Powered
            </Badge>
            ...
            <Card className="bg-gradient-to-br from-indigo-50 to-violet-50 border-indigo-200">
              <CardContent className="p-8 sm:p-12">
                <div className="flex flex-col items-center text-center">
                  ...
                  <Button size="lg" className="bg-gradient-to-r from-indigo-500 to-violet-500 text-white hover:opacity-90">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {tool.popular && (
                            <Badge variant="secondary" className="text-xs bg-indigo-100 text-indigo-700 border-indigo-200">Popular</Badge>
                          )}
                        </div>
          ...
          <div className="max-w-4xl mx-auto">
            <h2 className="text-2xl font-bold text-slate-900 text-center mb-8">Popular Use Cases</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {useCases.map((useCase) => (
                <div key={useCase.title} className="p-4 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 transition-all">
                  <h3 className="font-medium text-slate-900 text-sm">{useCase.title}</h3>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-4">Try It Now</h2>
          <p className="text-slate-600 mb-6 max-w-lg mx-auto">Upload an image and see the magic happen in seconds</p>
          <Link href={`/${locale}`}>
            <Button size="lg" className="bg-gradient-to-r from-indigo-500 to-violet-500 text-white hover:opacity-90">
