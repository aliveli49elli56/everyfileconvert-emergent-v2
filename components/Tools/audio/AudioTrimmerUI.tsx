...
export default function AudioTrimmerUI({ onFileSelected }: Props) {
  ...
  const loadFile = (f: File) => {
    ...
    setStage('idle'); setResult(null); setError(null); setStart(0);
    onFileSelected?.(f);
  };

  const handleMeta = () => {
    const d = audioRef.current?.duration ?? 60;
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const res = await Transcoder.run({
        files: [file], op: 'audio:trim',
        options: { startTime: start, endTime: end },
        onProgress: (pct) => { setProgress(pct); setStageMsg(pct < 25 ? 'Loading FFmpeg…' : 'Trimming audio…'); },
      });
  ...
  if (!file) return (
    <div
      ...
      data-testid="audio-trimmer-drop"
      className="group flex flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 px-8 py-14 cursor-pointer hover:border-orange-400 hover:bg-orange-50/60 transition-all"
    >
      <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white shadow-sm group-hover:shadow-md transition-shadow">
        <Scissors className="h-6 w-6 text-slate-400 group-hover:text-orange-500 transition-colors" />
      </div>
      <div className="text-center">
        <p className="text-sm font-semibold text-slate-700">Drop an audio file to trim</p>
        <p className="text-xs text-slate-400 mt-1">or <span className="text-orange-500">click to browse</span> — MP3, WAV, FLAC, AAC, OGG</p>
      </div>
  ...
  return (
    <div className="space-y-4" data-testid="audio-trimmer-editor">
      {stage === 'processing' ? (
        ...
        <>
          <audio ref={audioRef} src={objUrl!} controls className="w-full rounded-xl" onLoadedMetadata={handleMeta} />
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs text-slate-500">
              <span>Start: <span className="font-bold text-orange-600">{fmt(start)}</span></span>
              <span>Duration: <span className="font-bold text-slate-700">{fmt(end - start)}</span></span>
              <span>End: <span className="font-bold text-orange-600">{fmt(end)}</span></span>
            </div>
            <div className="relative h-8 flex items-center">
              <div className="absolute h-2 w-full rounded-full bg-slate-200" />
              <div className="absolute h-2 rounded-full bg-orange-400"
                style={{ left: `${(start / duration) * 100}%`, right: `${100 - (end / duration) * 100}%` }} />
              <input type="range" min={0} max={duration} step={0.1} value={start}
                onChange={e => { const v = Math.min(+e.target.value, end - 0.5); setStart(v); if (audioRef.current) audioRef.current.currentTime = v; }}
      ...
      <div className="flex items-center justify-between border-t border-slate-100 pt-3">
        ...
        {resultUrl
          ? <a href={resultUrl} download={`trimmed_${file?.name}`} data-testid="audio-trim-download"
          ...
          : <button onClick={handleProcess} disabled={stage === 'processing' || end <= start} data-testid="audio-trim-process"
            className="flex items-center gap-1.5 rounded-xl bg-orange-500 px-5 py-2.5 text-sm font-semibold text-white hover:bg-orange-600 disabled:opacity-40 transition-colors">
            {stage === 'processing' ? <><Loader2 className="h-4 w-4 animate-spin" />Trimming…</> : <><Scissors className="h-4 w-4" />Trim Audio</>}
          </button>}
      </div>
    </div>
  );
