...
export default function AudioExtractorUI({ toolName, onFileSelected }: Props) {
  ...
  const handleProcess=async()=>{
    ...
    try{
      ...
      const res=await Transcoder.run({
        files:[file],op:'video:extract-audio',
  ...
  if(!file)return(
    <div onDrop={e=>{e.preventDefault();const f=e.dataTransfer.files[0];if(f)loadFile(f);}} onDragOver={e=>e.preventDefault()}
      onClick={()=>fileInput.current?.click()} data-testid="extractor-drop"
      ...
      <div className="text-center"><p className="text-sm font-semibold text-slate-700">Drop a video to extract audio</p>
  ...
  return(
    <div className="space-y-4" data-testid="extractor-editor">
      {stage==='processing'?(
        ...
        <>
          <video src={objUrl!} controls muted className="w-full rounded-xl bg-black" style={{maxHeight:260}}/>
          <div className="space-y-2">
            <p className="text-sm font-medium text-slate-700">Output format</p>
            <div className="flex gap-2">{(['mp3','aac','wav'] as const).map(f=>(
              <button key={f} onClick={()=>setFormat(f)}
                className={`rounded-lg px-4 py-2 text-sm font-bold uppercase transition-all ${format===f?'bg-violet-500 text-white':'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>{f}</button>
            ))}</div>
          </div>
      ...
      <div className="flex items-center justify-between border-t border-slate-100 pt-3">
        ...
        {resultUrl
          ?<a href={resultUrl} download={`audio_${file?.name?.replace(/\.[^.]+$/,`.${format}`)}`} data-testid="extract-download"
              className="flex items-center gap-1.5 rounded-xl bg-emerald-500 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-600"><Download className="h-4 w-4"/>Download {format.toUpperCase()}</a>
          :<button onClick={handleProcess} disabled={stage==='processing'} data-testid="extract-process"
              ...
              {stage==='processing'?<><Loader2 className="h-4 w-4 animate-spin"/>Extracting…</>:'Extract Audio'}
            </button>}
      </div>
    </div>
  );
