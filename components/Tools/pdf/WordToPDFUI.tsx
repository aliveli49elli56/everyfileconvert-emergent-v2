...
export default function WordToPDFUI({ onFileSelected }: Props) {
  ...
  const loadFile = (f: File) => {
    const ok = f.name.endsWith('.docx') || f.name.endsWith('.doc');
    if (!ok) return;
  ...
  if (!file) return (
    ...
    >
      ...
      <div className="text-center">
        <p className="text-sm font-semibold text-slate-700">Drop a Word document to convert to PDF</p>
        <p className="text-xs text-slate-400 mt-1">or <span className="text-amber-500">click to browse</span> — .docx, .doc accepted</p>
      </div>
      <input ref={fileInput} type="file" accept={ACCEPTED} className="sr-only"
        onChange={e => { const f = e.target.files?.[0]; if (f) loadFile(f); }} />
    </div>
  );

  return (
    <div className="space-y-5" data-testid="word-to-pdf-editor">
      {stage === 'processing' ? (
        ...
        <>
          <div className="flex items-center gap-4 rounded-xl bg-slate-50 px-4 py-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-xs font-semibold text-slate-700">{file.name.replace(/\.docx?$/, '.pdf')}</p>
                <p className="text-xs text-rose-500 uppercase font-bold">PDF</p>
              </div>
            </div>
          </div>
